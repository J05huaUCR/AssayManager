...
Value::Value(const Value& v) : mValueType(v.mValueType)
{
	switch (mValueType)
	{
		...
		case UntVal			: mUntVal = v.mUntVal; mFloatVal = (float)v.mUntVal; mDoubleVal = (double)v.mUntVal; break;
...
std::string SerializeValue(const Value& v)
{
	...
	switch (v.GetType())
	{
		...
		case UntVal			: snprintf(buff, BUFF_SZ, "%d", (unsigned)v); str = buff; break;
